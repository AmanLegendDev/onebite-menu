...
useEffect(() => {
  ...
  async function handleStart(e) {
    ...
    if (name.trim().length < 2) return alert("Enter valid name");
    if (phone.trim().length < 10) return alert("Enter valid phone");
    ...
    {tableInfo?.number || "Loading..."}
        ...
        <form onSubmit={handleStart} className="mt-8 flex flex-col gap-4">
          <input
            ...
            className="bg-[#1c1c1c] p-3 rounded-md outline-none border border-[#333]"
            value={name}
